    ...
    def silver_to_gold(self, silver_path=None, output_path=None):
        ...
        try:
            ...
            df_daily_trend = df_silver \
                ...
                .agg(f"Writing Gold layer datasets (mode={mode})...")
            ...
            df_province_summary.write \
                ...
                .parquet(f"{output_path}/province_summary/")
            logger.info(f"  ✓ Province summary written")
            incremental_pipeline(self, year, month):
        """
...
def main():
    ...
    parser.add_argument('--phase', choices=['bronze-silver', 'silver-gold', 'full'],
                       default='full', help='ETL phase to run')
    parser.add_argument('--minio-endpoint', default='localhost:9000',
                       help='MinIO endpoint')
    parser.add_argument('--bucket', default='datalake',
                       help='MinIO bucket name')
    ...
    # Run requested phase
    if args.phase == 'bronze-silver':
        etl.bronze_to_silver()
    elif args.phase == 'silver-gold':
        etl.silver_to_gold()
    else:
