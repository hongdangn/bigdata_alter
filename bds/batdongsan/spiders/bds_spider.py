...
class BdsSpiderSpider(scrapy.Spider):
    ...
    def parse_real_estate_page(self, response):
        if "404-page" in response.url:
            return

        if not len(response.css('ul.breadcrumb li')):
            return

        if len(response.css('ul.list-attr-hot')) < 2:
            return

        if len(response.css("ul.list-attr-hot")[1].css('li')) < 3:
            return
        ...
        bds_item['num_bedrooms'] = response.css("ul.list-attr-hot")[1].css('li')[0].css('span.value-attr2::text').get()
        bds_item['num_toilets'] = response.css("ul.list-attr-hot")[1].css('li')[1].css('span.value-attr2::text').get()
        bds_item['num_floors'] = response.css("ul.list-attr-hot")[1].css('li')[2].css('span.value-attr2::text').get()
