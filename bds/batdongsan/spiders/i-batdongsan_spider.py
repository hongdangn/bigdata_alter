...
class IBatdongsanSpider(scrapy.Spider):
    ...
    def start_requests(self):
        start_url = 'https://i-batdongsan.com/'
    ...
    def parse_home_page(self, response):
        ...
        for province_page_link in province_page_links:
            province = '-'.join(province_page_link.split('/')[-1].replace('.htm', '').split('-')[0:-1])
            if province == self.province:
                province_full_page_link = urljoin('https://i-batdongsan.com/', province_page_link)
        ...
        if self.jump_to_page > 1:
            ...
            if page >= self.min_page:
                ...
                for real_estate in real_estates:
                    real_estate_link = real_estate.css('div.ct_title a::attr(href)').get()
                    real_estate_link = urljoin('https://i-batdongsan.com/', real_estate_link)
            ...
            if next_page is not None and page < self.max_page:
                next_page = urljoin('https://i-batdongsan.com/', next_page)
    ...
    def parse_real_estate_page(self, response):
        ...
        bds_item['title'] = response.css('div.title h1::text').get()
        ...
        bds_item['address']['full_address'] = response.css('div.address span.value::text').get()

        contact_info = {}
        contact_info['name'] = response.css('div.contact-info div.content div.name::text').get()
        contact_info['phone'] = [phone for phone in response.css('div.contact-info div.content div.fone a::text').getall()]
